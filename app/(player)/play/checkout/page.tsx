...
export default function CheckoutPage() {
  ...
  async function handleConfirmAndPay() {
    ...
    const [startStr, endStr] = draft.timeRange.split(' - ');
    const payload = {
      pitchId: draft.courtId,
      startTime: new Date(`${draft.date}T${startStr}:00`).toISOString(),
      endTime: new Date(`${draft.date}T${endStr}:00`).toISOString(),
      totalAmount: draft.totalDue,
    };

    const bookingPromise = apiRequest<{ id: string }>('/bookings', {
      method: 'POST',
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-10">
      <div className="mb-8">
        ...
        <p className="font-dm text-sm text-gray-400">Review your details and confirm payment to secure the pitch.</p>
      </div>
            ...
            <div className="rounded-2xl overflow-hidden bg-gray-900 mb-4">
              ...
              <div className="px-5 pb-5 space-y-3">
                ...
                <div className="border-t border-white/10 pt-3 flex items-center justify-between">
                  ...
                  <span className="font-syne font-bold text-green-400 text-lg">{formatCurrency(draft.totalDue)}</span>
                </div>
              </div>
            </div>
...
function PaymentOption({
  ...
  return (
    ...
    >
      ...
      {disabled && <span className="font-dm text-[10px] uppercase tracking-wide text-gray-400">Coming soon</span>}
    </button>
  );
}
